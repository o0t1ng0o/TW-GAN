...
def define_G(input_nc, guide_nc, output_nc, ngf, netG, n_layers=8, n_downsampling=3, n_blocks=9, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netG == 'bFT_unet_cat':
        net = bFT_Unet_cat(input_nc, guide_nc, output_nc, n_layers, ngf, norm_layer=norm_layer)
    elif netG == 'uFT_unet':
        net = uFT_Unet(input_nc, guide_nc, output_nc, n_layers, ngf, norm_layer=norm_layer)
    elif netG == 'concat_Unet':
        net = concat_Unet(input_nc, guide_nc, output_nc, n_layers, ngf, norm_layer=norm_layer)
    else:
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', use_sigmoid=False, init_type='normal', init_gain=0.02, gpu_ids=[], num_classes_D=1, use_noise=False, use_dropout=False):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)

    if netD == 'basic':
        net = NLayerDiscriminator(input_nc, ndf, n_layers=n_layers_D, norm_layer=norm_layer, use_sigmoid=use_sigmoid, num_classes_D=num_classes_D, use_noise=use_noise, use_dropout=use_dropout)
    elif netD == 'n_layers':
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, num_classes_D=1, use_noise=False, use_dropout=False):
        ...
        if use_dropout:
            sequence.append(nn.Dropout(p=0.2))
        ...
        #     nn.LeakyReLU(0.2, True)
        # ]
        sequence += [nn.Conv2d(ndf * nf_mult, num_classes_D, kernel_size=3, stride=1, padding=padw)]
        if use_sigmoid:
            sequence += [nn.Sigmoid()]
        self.model = nn.ModuleList(list(nn.Sequential(*sequence)))
    def forward(self, input, layers=None):
        input = input + torch.randn_like(input) if self.use_noise else input
        #output = self.model(input)
        output = input
        results = []
        for ii, model in enumerate(self.model):
            output = model(output)
            if layers and ii in layers:
                results.append(output.view(output.shape[0], -1))
        if layers == None:
...
class bFT_Unet(nn.Module):
    def __init__(self, input_nc, guide_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, bottleneck_depth=100):
        ...
        self.upconv5 = nn.Sequential(*[nn.ReLU(True), nn.ConvTranspose2d(ngf * 2, output_nc, kernel_size=4, stride=2, padding=1)])
...
class bFT_Unet_cat(nn.Module):
    def __init__(self, input_nc, guide_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, bottleneck_depth=100):
        super(bFT_Unet_cat, self).__init__()
...
class concat_Unet(nn.Module):
    def __init__(self, input_nc, guide_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d,
        ...
        self.downconv1 = nn.Sequential(*[nn.Conv2d(input_nc+guide_nc, ngf, kernel_size=4, stride=2, padding=1, bias=use_bias)])
    ...
    def forward(self, input, guide):
        ## downconv
        input = torch.cat((input, guide), dim=1)
        down1 = self.downconv1(input)
        #G_down1 = self.G_downconv1(guide)
